...
# Detecta la cara del gato y agrega bigote y gorro
def add_mustache_and_hat(img_path):
    ...
    for (x, y, w, h) in faces:
        ...
        mustache_x = x + ((w - mustache.width)  // 2) + 100
        mustache_y = (y + (h // 2)) + 150
        
        # Ajusta la posición y el tamaño del gorro
        hat = Image.open('somb.png')
        hat_width = w
        hat_height = int((h * 0.75))
        hat = hat.resize((hat_width, hat_height), Image.Resampling.LANCZOS)
        
        # Calcula la posición para colocar el gorro (encima de la cabeza)
        hat_x = x - 200
        hat_y = y - hat_height + 200
        
        # Convierte la imagen base a PIL para pegar el bigote y el gorro
        ...
        base_pil_image.paste(hat, (hat_x, hat_y), hat)
...
if __name__ == "__main__":
    ...
    if is_cat(img_path):
        ...
        result_image = add_mustache_and_hat(img_path)
        result_image.show()  # Muestra la imagen con el bigote y el gorro
        result_image.save('cat_with_mustache_and_hat.jpg')  # Guarda la imagen con el bigote y el gorro
    else:
